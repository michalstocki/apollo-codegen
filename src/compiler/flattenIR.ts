...
export class Record {
  ...
  addField(field: Field) {
    ...
    if (existingField) {
      ...
      this.fieldMap.set(responseKey, field);
    }
  }
}
...
export class TypeCase {
  ...
  private visitSelectionSet(selectionSet: SelectionSet) {
      ...
      switch (selection.kind) {
        case 'Field':
          ...
          if (this.default.possibleTypes.every(type => selectionSet.possibleTypes.includes(type))) {
            this.default.addField(selection);
          }
          break;
        case 'TypeCondition':
          this.visitSelectionSet(selection.selectionSet);
          break;
      }
    }
  }
  ...
  inspect() {
    return (
      ...
      this.records.map(
        record => `  ${inspect(record.possibleTypes)} -> ${inspect(record.fields.map(field => field.name))}\n`
      ).join('')
    );
  }
}
